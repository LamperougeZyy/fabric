...
import (
	"bytes"
	"fmt"
	"github.com/dshulyak/go-hotstuff"
...
func newConsensus(
	...
	prepare, err := store.GetTagHeader(hotstuff.PrepareTag)
	if err != nil {
		logger.Fatal("failed to load prepare header", zap.Error(err))
	}
	locked, err := store.GetTagHeader(hotstuff.LockedTag)
	if err != nil {
		logger.Fatal("failed to load locked header", zap.Error(err))
	}
	commit, err := store.GetTagHeader(hotstuff.DecideTag)
	if err != nil {
		logger.Fatal("failed to load commit header", zap.Error(err))
	}
	prepareCert, err := store.GetTagCert(hotstuff.PrepareTag)
	if err != nil {
	...
	return &consensus{
		...
		id:          id,
		replicas:    replicas,
		timeouts:    hotstuff.NewTimeouts(verifier, 2*len(replicas)/3+1),
		votes:       hotstuff.NewVotes(verifier, 2*len(replicas)/3+1),
		prepare:     prepare,
		locked:      locked,
		commit:      commit,
...
type consensus struct {
	...
	votes    *hotstuff.Votes
	timeouts *hotstuff.Timeouts
...
func (c *consensus) Send(state, root []byte, data *types.Data) {
	if c.waitingData {
		...
		c.sendMsg(hotstuff.NewProposalMsg(proposal))
	}
}
...
func (c *consensus) onProposal(msg *types.Proposal) {
	...
	if msg.Timeout != nil {
		if !c.verifier.VerifyAggregated(hotstuff.HashSum(hotstuff.EncodeUint64(msg.Header.View-1)), msg.Timeout.Sig) {
			return
		}
	}
...
func (c *consensus) sendNewView() {
	...
	nview := &types.NewView{
		...
		Sig: c.signer.Sign(nil, hotstuff.HashSum(hotstuff.EncodeUint64(c.view-1))),
	}
	c.voted = c.view - 1
	...
	c.sendMsg(hotstuff.NewViewMsg(nview), leader)
}
...
func (c *consensus) sendVote(header *types.Header) {
	...
	c.sendMsg(hotstuff.NewVoteMsg(vote), c.getLeader(vote.View+1))
}
...
func (c *consensus) update(parent *types.Header, cert *types.Certificate) {
	...
	if gparent.View > c.locked.View {
		...
		c.locked = gparent
		err := c.store.SetTag(hotstuff.LockedTag, gparent.Hash())
		if err != nil {
	...
	if parent.View-gparent.View == 1 && gparent.View-ggparent.View == 1 && ggparent.View > c.commit.View {
		...
		c.commit = ggparent
		err := c.store.SetTag(hotstuff.DecideTag, ggparent.Hash())
		if err != nil {
...
func (c *consensus) updatePrepare(header *types.Header, cert *types.Certificate) {
	if header.View > c.prepare.View {
		...
		c.prepare = header
		c.prepareCert = cert
		err := c.store.SetTag(hotstuff.PrepareTag, header.Hash())
		if err != nil {
